package com.miracle.app.uilogic.galanz;
...
public class GalanzUiLogic extends BaseHomeUiLogic implements IProtocolListener, View.OnClickListener {
    ...
    public GalanzUiLogic(Activity activity) {
        super(activity);
        initView();
        initData();
        initEvent();
    ...
    public void onNewUmdbData(UMDB db) {
        ...
        if (BrandConfig.DEVICE_MODEL.equals(BrandConfig.MODEL_XIAOTIANE)) {//小天鹅
            data[9] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData9));
            data[10] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData10));
            data[11] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData11));
            data[12] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData12));
            data[13] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData13));
            data[14] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData14));
            data[15] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData15));
            data[16] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData16));
            data[17] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData17));
            data[18] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData18));
            data[19] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData19));
            data[20] = StringUtils.parseInteger(newDb.get(UMDB.MotorAckData20));
        }
        mProtocolCmdLogic.recvResponse(error, cmdAck, data);
        mProtocolCmdLogic.recvResponse(error, cmdAck, db);
    }
    ...
    protected void initData() {
        //三洋
        mProtocolCmdLogic = GalanzProtocolLogic.getInstance();
